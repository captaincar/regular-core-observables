...
Isospectrality is preserved at the level accessible to leading-order WKB: the axial–polar splitting is $\sim 0.1\%$, two orders of magnitude below the method's own truncation error ($\sim 7\%$ absolute offset: WKB gives 208 Hz vs exact Schwarzschild 195 Hz). This splitting is a WKB artefact, not a physical signal. The Zerilli extension used here substitutes $m_{\rm eff}(r)$ for the constant $M$ in the standard Schwarzschild formula; a rigorous derivation from the perturbed stress-energy tensor has not been performed. The $\sim 0.1\%$ splitting is therefore best interpreted as an order-of-magnitude upper bound on any genuine isospectrality violation, not as a physical prediction. We report $|f_{\rm polar} - f_{\rm axial}|/f \sim O(10^{-3})$ (static, subject to WKB systematics).
...
| $10^{-12}$ m | pm / atomic | $\sim 10^{18}$ | Excluded by microlensing |
...
2. **Cross-metric completeness and isospectrality.** Dymnikova QNM shifts are confirmed to be $\approx 0$ (exponential suppression), but the echo-extremality coincidence should be verified numerically for Bardeen and Dymnikova. The axial–polar isospectrality splitting of $\sim 0.1\%$ ($O(10^{-3})$) is an order-of-magnitude estimate two orders below the leading-order WKB truncation floor; third-order Konoplya WKB or an exact numerical computation is needed to determine whether isospectrality is genuinely preserved at that level. The Zerilli potential extension to non-vacuum backgrounds must be derived from first principles before isospectrality claims can be made; the current substitution of $m_{\rm eff}(r)$ for the constant $M$ in the Schwarzschild formula is an ansatz whose validity has not been independently verified.
